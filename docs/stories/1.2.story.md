...
## Tasks / Subtasks
- [x] Implement SQLite connection (AC: 1)
  - [x] Ensure `apps/backend/internal/database/connection.go` initializes SQLite with WAL and foreign keys enabled [Source: docs/architecture/backend-architecture.md#database-integration]
  - [x] Confirm database file path and migrations path are configurable via `config.Database` (`DATABASE_URL`, `DATABASE_MIGRATIONS_PATH`) [Source: docs/architecture/backend-architecture.md#configuration-management]
- [x] Introduce versioned migrations runner (AC: 2, 4)
  - [x] Add a simple migration runner under `apps/backend/cmd/migrate` to execute `*.up.sql` and `*.down.sql` in order, tracking versions in a `schema_migrations` table [Source: docs/architecture/backend-architecture.md#migration-strategy]
  - [x] Provide commands: `migrate up` (to latest), `migrate down` (one step), and `migrate to <version>` [Source: docs/architecture/backend-architecture.md#migration-strategy]
  - [x] Update `Makefile` with `db-migrate-up` and `db-migrate-down` targets [Source: docs/architecture/development-workflow.md#development-process]
- [x] Create initial `users` table migration (AC: 3)
  - [x] Add `apps/backend/migrations/001_users_table.up.sql` implementing the `users` schema per architecture [Source: docs/architecture/database-schema.md#sqlite-schema-definition]
  - [x] Add `apps/backend/migrations/001_users_table.down.sql` to drop the table and associated indexes/triggers [Source: docs/architecture/backend-architecture.md#migration-strategy]
- [x] Apply and verify migration (AC: 4)
  - [x] Run `migrate up` locally to create `users`
  - [x] Add a database test using in-memory SQLite to run `up` then `down`, verifying table presence/absence [Source: docs/architecture/development-workflow.md#database-testing]
- [x] Documentation
  - [x] Add brief usage notes to `README.md` for migration commands [Source: docs/architecture/development-workflow.md#development-process]
...
| 2025-08-19 | 1.0 | Implemented DB connection, migration runner, users migration; fixed trigger handling; tests passing | James |

## Dev Agent Record
### Agent Model Used
James (dev) — Completed DB setup, migration runner, users migration; fixed trigger handling and validated with tests
...
- Fixed SQL execution to run full scripts atomically (supports triggers with BEGIN...END)
...
- Updated README with migration usage (make db-migrate-up/down)
 - Wired migration tests and CLI smoke test into CI workflow

### File List
- apps/backend/internal/migrate/runner.go (updated)
