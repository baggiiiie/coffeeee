...
## Tasks / Subtasks
- [ ] Backend: Implement `PUT /api/v1/users/me` (AC: 1)
  - [ ] In `apps/backend/internal/api/handlers/user.go#UpdateProfile`, read user ID from request context (JWT middleware)
  - [ ] Parse request body `{ "username"?: string, "email"?: string }` with validation
  - [ ] Validate: email format (`^[^\s@]+@[^\s@]+\.[^\s@]+$`), username pattern (`^[a-zA-Z0-9_-]{3,50}$`), at least one field provided
  - [ ] Check email uniqueness if email is being updated (exclude current user)
  - [ ] Update DB and return 200 with updated user JSON; handle 400/409/401 errors with specific messages
  - [ ] Set `Content-Type: application/json; charset=utf-8`
  [Source: docs/architecture/api-specification.md#user--authentication-endpoints] [Source: docs/architecture/error-handling-strategy.md#backend-error-handling]
- [ ] Backend: Ensure route wiring (AC: 1)
  - [ ] Confirm `protected.HandleFunc("/users/me", userHandler.UpdateProfile).Methods("PUT")` is enabled under protected subrouter (already present)
  [Source: apps/backend/internal/api/routes/routes.go]
- [ ] Frontend: Profile form component (AC: 2, 4)
  - [ ] Create `apps/frontend/src/components/ProfileForm.tsx` with Material-UI form fields for username and email
  - [ ] Implement real-time validation using specified regex patterns
  - [ ] Track form dirty state by comparing current values to original `AuthContext.user` values
  - [ ] Add loading state, submit/cancel buttons with `data-testid="submit-button"` and `data-testid="cancel-button"`
  - [ ] Handle unsaved changes warning using `react-router` `Prompt` component
  [Source: docs/architecture/frontend-architecture.md#react-component-hierarchy] [Source: docs/architecture/frontend-architecture.md#component-design-patterns]
- [ ] Frontend: Profile page integration (AC: 2, 3)
  - [ ] Update `apps/frontend/src/pages/UserProfilePage.tsx` to use `ProfileForm` and display current user data
  - [ ] Wire form submission to `PUT /api/v1/users/me` via API module
  - [ ] On success: update `AuthContext.user`, show success toast "Profile updated successfully", reflect in header
  - [ ] On error: show specific error toast with validation/conflict details
...
- [ ] Frontend: AuthContext update (AC: 3)
  - [ ] Add `updateProfile(profileData: UpdateUserRequest)` to `AuthContext` to update local user state
  - [ ] Ensure header reflects updated username/email immediately
  [Source: docs/architecture/frontend-architecture.md#state-management-strategy]
- [ ] Tests (AC: 1, 2, 3)
  - [ ] Backend integration tests: valid updates return 200, validation errors return 400 with specific messages, email conflict returns 409
  - [ ] Frontend validation tests: invalid email formats, username too short/long, empty submission, form dirty state tracking
  - [ ] Frontend submission tests: success/error states, AuthContext updates, header reflects changes, submit button enable/disable
