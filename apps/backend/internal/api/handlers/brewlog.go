...
func (h *BrewLogHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	if err := dec.Decode(&body); err != nil {
		...
		_ = json.NewEncoder(w).Encode(map[string]string{"code": "VALIDATION_ERROR", "message": errMsg})
		return
	}
	...
	if body.BrewMethod != nil {
		...
		if bm == "" {
			w.WriteHeader(http.StatusBadRequest)
			_ = json.NewEncoder(w).Encode(map[string]string{"code": "VALIDATION_ERROR", "message": "brewMethod cannot be empty"})
			return
		}
	}
	if body.CoffeeWeight != nil && (*body.CoffeeWeight < 0 || *body.CoffeeWeight > 200) {
